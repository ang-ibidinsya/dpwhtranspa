...
const prepareMyCategoryCard = (row, masterData, table, sortedIndex) => {
    let myCategory = row.getValue('myCategory');
    let categoryName = getMasterDataValue(masterData, EntityTypes.myCategory, myCategory);
    let subtotal = row.getValue('subtotal');
    let {entityGroups, minCost, maxCost, checkedStretch, pagination} = table.getState();
    const findEntity = entityGroups.find(grp => grp.myCategory === myCategory);
            ...
            <table className="tableInsideCard">
                ...
                <tr className="tableInsideCard-row">
                    ...
                    <td className="tableInsideCard-secondCol"><div className=''>
                            ...
                            >                        
                            <StackedBarChart name={`card-myCategory-${myCategory}`} 
...
export const CardContainerMyCategory = ({table, masterData, windowWidth, grandTotal}) => {
    const rows = table.getRowModel().rows;    
    let cardContainerTopRef = useRef(null)
    const rowElems = rows.map( (row, sortedIndex) => {
        return <div key={row.id}>
            {prepareMyCategoryCard(row, masterData, table, sortedIndex)}
        </div>
    })
